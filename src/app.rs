...
use crate::keyboard::{k884x, k8890, Keyboard, Key, KnobAction, MediaCode, Modifier, WellKnownCode};
use crate::options::{Command, ConfigParams, DevelOptions, LedCommand, Options};
...
pub fn execute(options: Options) -> Result<()> {
    match options.command {
        Command::ShowKeys => show_keys(),

        Command::Validate(params) => {
            let config: Config = load_config(&params).context("load mapping config")?;
            validate_config(config)
        }

        Command::Upload(params) => {
            let config: Config = load_config(&params).context("load mapping config")?;
            upload_config(config, &options.devel_options)
        }

        Command::Led(LedCommand { mut args }) => set_led(&mut args, &options.devel_options),
    }
}

pub fn show_keys() -> Result<()> {
    println!("Modifiers: ");
...
pub fn upload_config(config: Config, devel_options: &DevelOptions) -> Result<()> {
    let (buttons, knobs) = (config.rows * config.columns, config.knobs);
    let layers = config.render().context("render mapping config")?;

    let (handle, endpoint, id_product) = open_device(devel_options)?;
...
pub fn set_led(args: &mut Vec<String>, devel_options: &DevelOptions) -> Result<()> {
    let (handle, endpoint, id_product) = open_device(devel_options)?;
...
fn create_driver(id_product: u16, buttons: u8, knobs: u8) -> Result<Box<dyn Keyboard>> {
    let keyboard: Box<dyn Keyboard> = match id_product {
        0x8840 | 0x8842 | 0x8850 => Box::new(k884x::Keyboard884x::new(buttons, knobs)?),
        0x8890 => Box::new(k8890::Keyboard8890::new()),
        _ => unreachable!("unsupported device"),
    };
    Ok(keyboard)
}
